...
try {
	[string]$projectfile = Get-VstsInput -Name projectfile # Path to finalbuilder project. 
	[string]$cwd = Get-VstsInput -Name cwd # Path to current working directory.
	[string]$dropfolder = Get-VstsInput -Name dropfolder # Path to network location to drop built artifacts.
	[string]$targets = Get-VstsInput -Name Targets # List of targets to run with in the FinalBuilder project.
	[string]$solutionfile = Get-VstsInput -Name Solutionfile # The solution file search string for the solution(s) to build.
	[string]$platform = Get-VstsInput -Name Platform # The platform to build the solutions under (x86,x64,Any CPU).
	[string]$flavor = Get-VstsInput -Name Flavor # The flavor of build to perform on the solutions (Debug, Release, Etc).
	[string]$customArgs = Get-VstsInput -Name CustomArgs # A line seperated list of custom arguments for FinalBuilder. 
	[boolean]$includeChangesetsInTriggerFile = Get-VtsInput -Name IncludeChangesetsInTriggerFile
	...
	if ($includeChangesetsInTriggerFile){

		$fromDate = Get-Date.AddDays(-7) -format "dd-MM-yyyy HH:mm:ss"
